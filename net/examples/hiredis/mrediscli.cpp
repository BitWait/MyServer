...
{
  c->command(std::bind(echoCallback, std::placeholders::_1, std::placeholders::_2, s), "echo %s", s->c_str());
}
...
int main(int argc, char** argv)
{
  ...
  string hi = "hi";
  hiredis.command(std::bind(echoCallback, std::placeholders::_1, std::placeholders::_2, &hi), "echo %s", hi.c_str());
  loop.runEvery(2.0, std::bind(echo, &hiredis, &hi));

  hiredis.command(dbsizeCallback, "dbsize");

  uint16_t index = 8;
  hiredis.command(std::bind(selectCallback, std::placeholders::_1, std::placeholders::_2, &index), "select %d", index);

  string password = "password";
  hiredis.command(std::bind(authCallback, std::placeholders::_1, std::placeholders::_2, &password), "auth %s", password.c_str());
