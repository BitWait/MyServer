...
void Hiredis::setChannel()
{
  ...
  channel_->setReadCallBack(std::bind(&Hiredis::handleRead, this, std::placeholders::_1));
...
void Hiredis::handleRead(Timestamp receiveTime)
{
...
int Hiredis::command(const CommandCallback& cb, std::string cmd, ...)
{
...
int Hiredis::ping()
{
  return command(std::bind(&Hiredis::pingCallback, this, std::placeholders::_1, std::placeholders:: _2), "PING");
}
